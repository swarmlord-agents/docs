...
```mermaid
flowchart LR
    create(["createSession"]):::call --> send(["send"]):::call --> stream(["stream events"]):::call --> completed(["session.completed"]):::terminal

    subgraph branches ["while a turn is in flight"]
        direction TB
        fork["new session<br/>shared history"]:::result
        aborted["session.aborted"]:::terminal
        replay["replay missed events"]:::result
    end
    ...
    classDef call fill:#ff5d73,stroke:#e63e5c,color:#fff,font-weight:bold;
