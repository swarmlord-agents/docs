...
```mermaid
flowchart LR
    create([createSession]) --> send([send])
    send --> stream([stream events])
    stream --> completed([session.completed]):::terminal

    send -. fork .-> fork([new session, shared history])
    send -. abort .-> aborted([session.aborted]):::terminal
    send -. "events?sinceSeq=N" .-> replay([replay])

    classDef terminal fill:#ff5d73,stroke:#e63e5c,color:#fff;
```
